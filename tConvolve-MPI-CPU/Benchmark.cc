...
void allocComputeArrays(const int samplesSize, const int gSize)
{
    Stopwatch sw;
    ...
    double ut = sw.stop();
    long caSize = samplesSize * (sizeof(int) * 2 + sizeof(double) * 2) + sizeof(int) * gSize;
...
void check_sg_size(const int new_sg_size)
{
    ...
    memset(sg_r, 0, sizeof(double) * new_sg_size);
    memset(sg_i, 0, sizeof(double) * new_sg_size);
}

void freeComputeArrays()
{
    free(iu2);
    free(cOffset2);
    free(data_r2);
...
void Benchmark::runGrid()
{
    ...
    allocComputeArrays(maxSamples, gSize);
...
void locateEdge(int *iv, int *iu, int samplesSize, int *above_v, 
                int gSize, int &minu, int &maxu, int &minv, int &maxv, int &lenu, int &lenv)
{
...
(
    int samplesSize, int *above_v, int minv, int lenv, int minu,
    int *iu, int *iv, int *cOffset, double *data_r, double *data_i,
    int *iu2, int *cOffset2, double *data_r2, double *data_i2
)
{
...
void gridInnerKernel
(
    int curr_v, int *above_v, double *sg_r, double *sg_i, 
    int lenv, int small_grid_lenu, int sSize, int *samples_iu, 
    int *samples_cOffset, double *samples_data_r, 
    double *samples_data_i, double *C_r, double *C_i
)
{
    ...
    //printf("Inner kernel : %d, %d -> %d ... done\n", curr_v, start_v, end_v);
}
...
void write_sg_back_to_grid(const int small_grid_lenv, const int small_grid_lenu, 
                           const int minv, const int minu, const int gSize,
...
)
{
    ...
    sw.start();
    resortSamplesByV(curr_samplesSize, above_v, minv, lenv,
                     minu, iu, iv, cOffset, data_r, data_i, 
    ...
    // Compute kernel
    #pragma omp parallel for schedule(dynamic)
    for (int curr_v = 0; curr_v < small_grid_lenv; curr_v++)
        gridInnerKernel(curr_v, above_v, sg_r, sg_i, lenv, 
                        small_grid_lenu, sSize, iu2, 
                        cOffset2, data_r2, data_i2, C_r, C_i);

    write_sg_back_to_grid(small_grid_lenv, small_grid_lenu, minv,  
                          minu, gSize, grid_r, grid_i, sg_r, sg_i);
}
